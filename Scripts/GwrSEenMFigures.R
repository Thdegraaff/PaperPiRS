...
## What:        Make figures of GWR of Energy and manufacturing Sector
## Edited:      20-04-2015
####################################################################################################
library("spdep")
library("maptools")
...
RCMat = read.csv("Data/Derived/SEenMRCMat.csv", header = FALSE)
...
Regions$SEeMDep[is.na(Regions$SEenMDep)] <- 0.01

Regions$sEM <- log(Regions$sEM)
Regions$sEM[is.na(Regions$sEM)] <- -10

Regions$nEM <- log(Regions$nEM)
Regions$nEM[is.na(Regions$nEM)] <- -10

formula <- log(SEenMDep) ~ log(SEenM0) + sEM + nEM + bevtot + popdens + hoogopl + RenDpub + Patent + RenDbus + Bbweg + Bblucht + Cong
...
for (i in varlist) {
    ...
    # tempplot <- spplot(gwr.res$SDF,"value",col.regions=myPal,cuts=8, main=paste("GWR using RC coefficient",i,"on EenM",sep=" "))
    tempplot <- spplot(gwr.res$SDF,"value",col.regions=myPal,cuts=8, at = seq(-0.15, 0.15, by = 0.005))
    pdf(paste("Output/Fig/RCEenM",i,".pdf", sep=""), height = 6)
    print(tempplot)
    dev.off()
    DataTemp <- data.frame(Regions$gamscode,gwr.res$SDF$value)
    colnames(DataTemp) <- c("Gamscode","Value GWR") 
    write.table(DataTemp, file = paste("Data/DataRoxana/RCEenM",i,".csv", sep=""), row.names = FALSE ,sep = ",")
}
####################################################################################################
...
#     tempTV <- eval(parse(text = tempTV))    
#     gwr.res$SDF$value <- (abs(tempTV) >= 1.96) * temp
#     tempplot <- spplot(gwr.res$SDF,"value",col.regions=myPal,cuts=8, main=paste("GWR using copatents coefficient",i,"on EenM",sep=" "))
#     jpeg(paste("Output/Fig/CoPatentEenM",i,".jpg", sep=""))
#     print(tempplot)
#     dev.off()
# }
# 
...
#     tempTV <- eval(parse(text = tempTV))    
#     gwr.res$SDF$value <- (abs(tempTV) >= 1.96) * temp
#     tempplot <- spplot(gwr.res$SDF,"value",col.regions=myPal,cuts=8, main=paste("GWR using FDI coefficient",i,"on EenM",sep=" "))
#     jpeg(paste("Output/Fig/FDIEenM",i,".jpg", sep=""))
#     print(tempplot)
#     dev.off()
# }
...
for (i in varlist) {
    ...
    tempTV <- eval(parse(text = tempTV))    
    gwr.res$SDF$value <- (abs(tempTV) >= 1.96) * temp
    # tempplot <- spplot(gwr.res$SDF,"value",col.regions=myPal,cuts=8, main=paste("GWR using Distance coefficient",i,"on EenM",sep=" "))
    tempplot <- spplot(gwr.res$SDF,"value",col.regions=myPal,cuts=8, at = seq(-0.15, 0.15, by = 0.005))
    pdf(paste("Output/Fig/DistanceEenM",i,".pdf", sep=""), height = 6)
    print(tempplot)
    dev.off()
    DataTemp <- data.frame(Regions$gamscode,gwr.res$SDF$value)
    colnames(DataTemp) <- c("Gamscode","Value GWR") 
    write.table(DataTemp, file = paste("Data/DataRoxana/DistanceEenM",i,".csv", sep=""), row.names = FALSE ,sep = ",")
}
