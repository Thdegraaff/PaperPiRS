...
## What:        Make figures of GWR of Construction Sector
## Edited:      20-04-2015
####################################################################################################
library("spdep")
library("maptools")
...
RCMat = read.csv("Data/Derived/SConsRCMat.csv", header = FALSE)
...
Regions$SConsDep[Regions$SConsDep <= 0] <- 0.01

Regions$sCon <- log(Regions$sCon)
Regions$sCon[is.na(Regions$sCon)] <- -10

Regions$nCon <- log(Regions$nCon)
Regions$nCon[is.na(Regions$nCon)] <- -10

formula <- log(SConsDep) ~ log(SCons0) + sCon + nCon + bevtot + popdens + hoogopl + RenDpub + Patent + RenDbus + Bbweg + Bblucht + Cong
...
for (i in varlist) {
    ...
    # tempplot <- spplot(gwr.res$SDF,"value",col.regions=myPal,cuts=8, main=paste("GWR using RC coefficient",i,"on Construction",sep=" "))
    tempplot <- spplot(gwr.res$SDF,"value",col.regions=myPal,cuts=8, at = seq(-0.15, 0.15, by = 0.005))
    pdf(paste("Output/Fig/RCCons",i,".pdf", sep=""), height = 6)
    print(tempplot)
    dev.off()
    DataTemp <- data.frame(Regions$gamscode,gwr.res$SDF$value)
    colnames(DataTemp) <- c("Gamscode","Value GWR") 
    write.table(DataTemp, file = paste("Data/DataRoxana/RCCons",i,".csv", sep=""), row.names = FALSE ,sep = ",")
}
####################################################################################################
...
#     tempTV <- eval(parse(text = tempTV))    
#     gwr.res$SDF$value <- (abs(tempTV) >= 1.96) * temp
#     tempplot <- spplot(gwr.res$SDF,"value",col.regions=myPal,cuts=8, main=paste("GWR using copatents coefficient",i,"on Construction",sep=" "))
#     jpeg(paste("Output/Fig/CoPatentCons",i,".jpg", sep=""))
#     print(tempplot)
#     dev.off()
# }
# 
...
#     tempTV <- eval(parse(text = tempTV))    
#     gwr.res$SDF$value <- (abs(tempTV) >= 1.96) * temp
#     tempplot <- spplot(gwr.res$SDF,"value",col.regions=myPal,cuts=8, main=paste("GWR using FDI coefficient",i,"on Construction",sep=" "))
#     jpeg(paste("Output/Fig/FDICons",i,".jpg", sep=""))
#     print(tempplot)
#     dev.off()
# }
...
for (i in varlist) {
    ...
    tempTV <- eval(parse(text = tempTV))    
    gwr.res$SDF$value <- (abs(tempTV) >= 1.96) * temp
    # tempplot <- spplot(gwr.res$SDF,"value",col.regions=myPal,cuts=8, main=paste("GWR using Distance coefficient",i,"on Construction",sep=" "))
    tempplot <- spplot(gwr.res$SDF,"value",col.regions=myPal,cuts=8, at = seq(-0.15, 0.15, by = 0.005))   
    pdf(paste("Output/Fig/DistanceCons",i,".pdf", sep=""), height = 6)
    print(tempplot)
    dev.off()
    DataTemp <- data.frame(Regions$gamscode,gwr.res$SDF$value)
    colnames(DataTemp) <- c("Gamscode","Value GWR") 
    write.table(DataTemp, file = paste("Data/DataRoxana/DistanceCons",i,".csv", sep=""), row.names = FALSE ,sep = ",")
}
