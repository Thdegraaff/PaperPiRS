	...
	****************************************************
	...
	import excel "${datafiles}EmploymentCE.xlsx", ///
	sheet(`EVar') cellrange(A1:M379) firstrow clear
	****************************************************
	************* Manage data **************************
	****************************************************			
	forvalues x = 2000/2010 {
		gen `EVar'E`x' = real(t`x')
		drop t`x'
	}
	replace Region = strupper(Region)
	save "${statafiles}RiE_`EVar'E06.dta", replace
	****************************************************
	************** Merge data **************************
	****************************************************		
	use "${statafiles}RiE_gamscode01.dta", replace
	merge m:1 Region using "${statafiles}RiE_`EVar'E06.dta"
	drop if _merge==2
